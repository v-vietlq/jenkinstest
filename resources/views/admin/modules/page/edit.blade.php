...
@section('title',__('module.title.page_edit'))
        ...
        <div class="col-xl-6">
            @card(['title' => 'module.card.infor_vi'])
                <div class="form-group">
                    ...
                    <input type="text" name="title_vi" class="form-control" value="{{ old('title_vi',$page->title_vi) }}" />
                </div>

                @editor(['name' => 'intro_vi','title' => 'form.page.intro_vi'])
                    {{ old('intro_vi',$page->intro_vi) }}
                @endeditor

                @editor(['name' => 'content_vi','title' => 'form.page.content_vi'])
                    {{ old('content_vi',$page->content_vi) }}
                @endeditor

                <div class="form-group">
                    <label>{{ __('form.page.slug_vi') }}: <span class="text-danger">*</span></label>
                    <input type="text" name="slug_vi" class="form-control" value="{{ old('slug_vi',$page->slug_vi) }}" />
                </div>

                <div class="form-group">
                    <label>{{ __('form.page.title_tag_vi') }}: <span class="text-danger">*</span></label>
                    <input type="text" name="title_tag_vi" class="form-control" value="{{ old('title_tag_vi',$page->title_tag_vi) }}" />
                </div>

                @description(['name' => 'description_tag_vi','title' => 'form.page.description_tag_vi'])
                    {{ old('description_tag_vi',$page->description_tag_vi) }}
                @enddescription
            @endcard
        </div>
        ...
        <div class="col-xl-6">
            @card(['title' => 'module.card.infor_en'])
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="slug_en" class="form-control" value="{{ old('slug_en',$page->slug_en) }}" />
                </div>

                <div class="form-group">
                    <label>{{ __('form.page.title_tag_en') }}: <span class="text-danger">*</span></label>
                    <input type="text" name="title_tag_en" class="form-control" value="{{ old('title_tag_en',$page->title_tag_en) }}" />
                </div>

                @description(['name' => 'description_tag_en','title' => 'form.page.description_tag_en'])
                    {{ old('description_tag_en',$page->description_tag_en) }}
                @enddescription
            @endcard
        </div>
    </div>
        ...
        <div class="col-xl-12">
            @card(['title' => 'module.card.information'])
                @switchCom(['name' => 'status','title' => 'form.page.status','on' => 'On','off' => 'Off'])
                    {{ (old('status',$page->status) == 'on') ? 'checked' : '' }}
                @endswitchCom
            @endcard
        </div>
    </div>
