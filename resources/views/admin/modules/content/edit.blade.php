...
@section('content')
<form action="{{ route('admin.content.update',['pageid' => $pageid,'id' => $id]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('POST')
    ...
    <div class="row">
        <div class="col-xl-6">
            @card(['title' => 'module.card.infor_vi'])
                <div class="form-group">
                    ...
                    <input type="text" name="content_vi" class="form-control" value="{{ old('content_vi',$content['content_vi']) }}" />
                </div>
            @endcard
        </div>
        ...
        <div class="col-xl-6">
            ...
            <div class="form-group">
                ...
                <input type="text" name="content_en" class="form-control" value="{{ old('content_en',$content['content_en']) }}" />
            </div>
            @endcard
        </div>
    </div>
            ...
            @card(['title' => 'module.card.information'])
                <div class="form-group">
                    ...
                    <select name="page_id" class="form-control">
                        ...
                        <option value="{{ $page->id }}" {{ (old('page_id',$content->page_id) == $page->id) ? 'selected' : '' }}>{{ $page->title_vi }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="code" class="form-control" value="{{ old('code',$content->code) }}" />
                </div>
            @endcard
        </div>
    </div>
