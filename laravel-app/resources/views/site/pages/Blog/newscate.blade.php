                ...
                <div id="jf-twocolumns" class="jf-twocolumns">
                    <div class="col-xs-12 col-sm-12 col-md-8 col-lg-9 pull-right">
                        <div id="jf-content" class="jf-content">
                            ...
                            {{--  Phân trang --}}
                            @if($news_by_cate->currentPage() != 1)
                            <nav class="jf-pagination">
                                <ul>
                                    @if($news_by_cate->currentPage() != 1)
                                    <li class="jf-prevpage"><a href="{{ $news_by_cate->url($news_by_cate->currentPage() - 1)}}"><i class="fa fa-angle-left"></i> Previous</a></li>
                                    @endif
                                    @for($i=1 ; $i <= $news_by_cate->lastPage(); $i = $i + 1)
                                        <li class="{{ ($news_by_cate->currentPage() == $i) ? 'jf-active' : '' }}"><a href="{{ $news_by_cate->url($i)}}">{{ $i }}</a></li>
                                    @endfor
                                    @if($news_by_cate->currentPage() != $news_by_cate->lastPage())
                                    <li class="jf-nextpage"><a href="{{ $news_by_cate->url($news_by_cate->currentPage() + 1)}}">Next <i class="fa fa-angle-right"></i></a></li> 
                                    @endif
                                </ul>
                            </nav>
                            @endif
